...
use tonic::transport::{Channel, Endpoint, channel::ClientTlsConfig};

pub struct JitoClient<S> {
    client: SearcherServiceClient<S>,
    endpoint: &'static str,
}

impl JitoClient<Channel> {
...
impl JitoClient<InterceptedService<Channel, ClientInterceptor>> {
    ...
    pub async fn new_authenticated(
        ...
        let timeout_dur = Duration::from_secs(timeout.unwrap_or(2));
        let auth_channel = Endpoint::from_shared(endpoint)?
            .tls_config(ClientTlsConfig::new().with_native_roots())?
            .tcp_nodelay(true)
            .timeout(timeout_dur)
            .connect_timeout(timeout_dur)
        ...
        let searcher_client = SearcherServiceClient::with_interceptor(channel, client_interceptor);

        Ok(Self {
            client: searcher_client,
            endpoint,
        })
    }
    pub fn get_endpoint(&self) -> &'static str {
...
mod tests {
    ...
    const SERVER_URL2: &str = "https://ny.mainnet.block-engine.jito.wtf:443";
    ...
    #[tokio::test]
    #[serial]
    async fn custom_endpoint_default_timeout_authenticated() {
        let full: [u8; 64] = [0; 64];
        let mut secret = [0u8; 32];
        secret.copy_from_slice(&full[..32]);

        let keypair = Keypair::new_from_array(secret);
        println!("{}", keypair.pubkey());

        match JitoClient::new_authenticated(SERVER_URL2, None, keypair).await {
            Ok(client) => println!("Get Endpoint: {}", client.get_endpoint()),
            Err(e) => panic!("Error in creating client: {e}"),
        }
    }
